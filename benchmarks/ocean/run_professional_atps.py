...
OUTER_GRACE=5
SOLVERS={
 'Vampire': lambda p: [os.environ.get('VAMPIRE_BIN','vampire'),'--mode','casc','-t',str(TIME_LIMIT),'-p','tptp',str(p)],
 ...
 'Prover9': lambda p: [os.environ.get('PROVER9_BIN','prover9'),'-tptp','-tptp_out','-t',str(TIME_LIMIT),'-f',str(p)],
}
...
def run_one(name,p,outdir):
    ...
    try:
        cp=subprocess.run(cmd,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=TIME_LIMIT+OUTER_GRACE,stdin=subprocess.DEVNULL)
    ...
    return {
        'solver':name,'file':p.name,'status':status_from_output(name,text,rc,timed),
        'wall_s':wall,'returncode':rc,'native_inference_records':native_inference_count(text),
        'time_limit_s':TIME_LIMIT,'command':' '.join(cmd)
    }
