#!/usr/bin/env python3
"""
Predator_4 -- learning to RANK premises over ZFC.   Brian Tenneson, v4.0
...
def rank_fit(pairs, epochs=300, lr=0.5, l2=1e-4, seed=0):
    """Learning to RANK, rather than to classify.

    The previous versions trained a binary classifier on (goal, candidate)
    pairs and then ranked by its score.  That is a mismatch: the classifier is
    optimised for "is this a premise?", while everything measured -- recall@k,
    effort, MRR -- depends only on the ORDER of candidates within one goal.  A
    model can classify most pairs correctly and still order the plausible ones
    badly, and ordering is the only thing a prover consumes.

    Here the training example is a PAIR: a true premise p and a non-premise n
    of the SAME goal.  We fit w so that w.x_p > w.x_n, by logistic regression
    on the difference vector x_p - x_n with label 1.  This is RankNet with a
    linear scorer.  Three consequences:

      * the objective is now exactly the quantity being reported;
      * scores become comparable only WITHIN a goal, which is all that is
        needed and all that was ever meaningful;
      * features constant across a goal -- the bias, anything depending on the
        goal alone -- cancel in the difference and cannot absorb weight.  The
        third point alone removes a class of bug that cost two versions.
    """
    if not pairs:
        raise SystemExit("no training pairs")
    D = [[a-b for a, b in zip(xp, xn)] for xp, xn in pairs]
    y = [1.0]*len(D)
    # antisymmetry: include the reversed difference with label 0, so the fit
    # cannot satisfy the objective by inflating every score.
...
class RankModel:
    """Linear scorer fitted by pairwise ranking loss.  No standardisation is
    needed: differences are already centred, and scale only rescales w."""
    name = "rank"
    def __init__(self, seed=0, **kw): self.seed = seed; self.w = None
    def fit_pairs(self, pairs):
        self.w = rank_fit(pairs, seed=self.seed); return self
    def fit(self, X, y): raise RuntimeError("RankModel is fitted with fit_pairs")
    def score(self, rows): return [dot(r, self.w) for r in rows]
    def describe(self, names): return sorted(zip(names, self.w), key=lambda kv: -abs(kv[1]))
...
class Predator4:
    ...
    def features(goal, cand, usage, rare, order_gap, neigh, cocite=0.0):
        ...
        return [1.0,
                ...
                math.log1p(neigh)/5.0,
                # Co-citation: lemmas that habitually appear TOGETHER in proofs.
                # If a goal's notation already points at premise A, and B is
                # cited alongside A across the library, B is a good bet even
                # when B shares no notation with the goal.  This is the one
                # signal that can reach premises stated in DIFFERENT notation --
                # the failure mode diagnosed on prcom, where the needed lemma
                # `uncom` mentions union and the goal does not.
                math.log1p(cocite)/5.0]
    ...
    def train(self, C, cut, n_neg=25, max_goals=4000, seed=0, ref_cut=None):
        ...
        PROTOCOL.  `cut` says which goals may be trained on.  `ref_cut` says
        which prefix the corpus STATISTICS -- citation counts, rare symbols,
        co-citation -- are computed from, and it defaults to `cut`.

        Separating the two matters for any experiment that varies the amount of
        training data.  Features like "log usage so far" and "shares rare
        symbol" are defined relative to a corpus; computing them over 1,600
        statements while fitting and over 25,600 while scoring means the same
        column denotes different quantities in the two phases, and the fitted
        weights are meaningless.  Holding `ref_cut` fixed across sizes makes a
        scaling curve measure the amount of training data and nothing else.
        """
        ...
        for g in goals:
            ...
            # every (premise, non-premise) pair from the SAME goal
            for xp in pos_feats:
                for xn in neg_feats:
                    pairs.append((xp, xn))
            seen += 1
        if not pairs: raise SystemExit("no training pairs; try a larger corpus")
        ...
        return dict(goals=seen, pairs=len(pairs), examples=len(pairs)*2,
                    positives=len(pairs), model="rank")

    @staticmethod
    def cocitation(C, cut, top=4000):
        """How often each pair of statements is cited by the same proof."""
    ...
    def cocite_score(pair, cand_label, anchors):
        """Total co-citation of a candidate with the goal's strongest anchors."""
        s = 0
        for a in anchors:
...
def evaluate(C, cut, pred, n_goals=200, pool_cap=3000, ks=(1, 5, 10, 50), seed=0,
    ...
    if st:                       # the SAME statistics the model was fitted under
...
def rank_for_goal(C, cut, pred, goal_idx, pool_cap=0):
    ...
    usage, rare = defaultdict(int), Predator4._rare_symbols(C)
    for t in C[:cut]:
        for p in t.premises: usage[p] += 1
    g = C[goal_idx]
    ...
    neigh = Predator4.local_use(C, goal_idx)
    st = getattr(pred, "stats", None)
    pair_tab = st["pair"] if st else Predator4.cocitation(C, cut)
    if st: usage, rare = st["usage"], st["rare"]
...
def cmd_prove(a):
    ...
    say("\n  training on statements 0..%d  (p = %.2f of everything before it)"
        % (cut-1, a.p))
    pred = Predator4(seed=a.seed, model=a.model)
...
def cmd_scale(a):
    ...
    for n in sizes:
        ...
        pred = Predator4(seed=a.seed, model=a.model)
        # ref_cut FIXED at hold_from for every size, so the curve measures the
        # amount of training data and not a shifting definition of the features
        info = pred.train(C_all[:biggest], cut, n_neg=a.n_neg,
                          max_goals=a.max_goals, seed=a.seed, ref_cut=hold_from)
...
class Run:
    ...
    def finish(self, res, pred=None, args=None):
        ...
        if pred: json.dump(pred.to_dict(),
                           open(os.path.join(self.dir, "predator_4.json"), "w"), indent=2)
...
def cmd_train(a):
    run = None if a.no_artifacts else Run(a.outdir)
    say = run.log if run else print
    say("="*70); say("  PREDATOR_4 v%s  --  premise selection over ZFC at scale" % VERSION)
    ...
    pred = Predator4(seed=a.seed, model=a.model, **chosen)
    t0 = time.perf_counter()
    info = pred.train(C, cut, n_neg=a.n_neg, max_goals=a.max_goals, seed=a.seed)
    ...
    for k, v in pred.model.describe(Predator4.FEATURES)[:6]:
    ...
    say("    %-10s %10s %10s %10s %10s"
        % ("", "Predator_4", "frequency", "chrono", "FUSED"))
    ...
    say("      Predator_4 %.4f   brute force %.4f   FUSED %.4f"
