...
1. **set.mm itself.** Everything above is the propositional core. The chronological-prefix protocol is written and validated but has not been run against the real database; it is a ~50 MB download that could not be retrieved in this environment. This is the single largest gap, and the two commands in §7 close it.
2. **A comparison baseline.** 4/16 is uninterpretable in isolation. On the same Metamath test-set task, Holophrasm reported 14.3% over 2,720 set.mm theorems and GPT-f reported roughly 56%. Those are full-database numbers with premise selection and are not directly comparable to a four-axiom propositional base, but no number here should be quoted without a baseline beside it.
3. **A cactus plot.** Requires enough solved instances to have a distribution; four is not enough. It becomes meaningful on set.mm.
4. **The `rank` intervention.** The dead heuristic slot is identified but not filled. A working ordering function is the obvious next experiment, and this harness measures its effect directly — rerun §3 with `rank` supplied and compare.
